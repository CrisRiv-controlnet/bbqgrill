...
<div class="order-paycol-inner">
    <div class="d-flex align-items-center justify-content-between payment-ttl-row">
        <div class="payment-ttl-left">
            @if($plan->shipping_method == 'on')
                ...
                {{ __('Shipping') }} :
                    <span class="payment-shipping">
                        ...
                        <span class="shipping_final_price"> - <b>{{ currency_format_with_sym((0 ?? 0) , $store->id, $currentTheme) ?? SetNumberFormat(0) }}</b>
                        </span>
                    </span>
                </span>
            @endif
            ...
            <span>
                {{ __('Coupon') }}:
                <b class="discount_amount_currency">- {{ currency_format_with_sym((0 ?? 0) , $store->id, $currentTheme) ?? SetNumberFormat(0) }}</b>
            </span>
        </div>
        <div class="payment-ttl-left">
            <h5> {{ __('Total') }} :</h5>
