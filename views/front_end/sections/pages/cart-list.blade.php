...
<div class="container">
    <div class="section-title">
        ...
        <h2> {{  $page_json->cart_page->section->title->text ?? __('Fashion category') }}<span>({{ $response->data->cart_total_product }})</span></h2>
        <p>{{$page_json->cart_page->section->title->text ?? ''}}</p>
    </div>
                        ...
                        @if (!empty($response->data->cart_total_product))
                            @foreach ($response->data->product_list as $item)
                                <tr>
                                    ...
                                    <td data-label="quantity">
                                        <div class="qty-spinner">
                                            ...
                                            <input type="text" class="quantity 45_quatity"
                                                ...
                                                id="cart_list_quantity{{ $item->qty }}">
                                    ...
                                    <td data-label="Price">
                                        {{ currency_format_with_sym($item->final_price / $item->qty ?? 0, $store->id, $currentTheme) ?? SetNumberFormat($item->final_price / $item->qty) }}
                                    </td>
                                    <td data-label="Total">
                                        {{ currency_format_with_sym($item->final_price ?? 0, $store->id, $currentTheme) ?? SetNumberFormat($item->final_price) }}
                                    </td>
                                    <td>

                                        <a class="remove_item_from_cart" title="Remove item"
                ...
                <ul>
                    <li>
                        <span class="cart-sum-left">{{ $response->data->cart_total_product }}
                            {{ __('item') }}</span>
                        <span
                            class="cart-sum-right">{{ currency_format_with_sym($response->data->final_price ?? 0, $store->id, $currentTheme) ?? SetNumberFormat($response->data->final_price ?? ($response->data->sub_total ?? 0)) }}</span>
                    </li>
                    <li>
                        <span class="cart-sum-left">{{ __('Taxes') }}: </span>
                        <span
                            class="cart-sum-right">{{ currency_format_with_sym($response->data->tax_price ?? 0, $store->id, $currentTheme) ?? SetNumberFormat($response->data->tax_price) }}</span>
                    </li>
                    <li>
                        <span class="cart-sum-left">{{ __('Discount') }}: </span>
                        <span
                            class="cart-sum-right coupon_discount_amount">{{ currency_format_with_sym(0 ?? 0, $store->id, $currentTheme) ?? SetNumberFormat() }}</span>
                    </li>
                    @php
                    ...
                    <li>
                        ...
                        <span
                            class="cart-sum-right discount_final_price">{{ currency_format_with_sym($final ?? 0, $store->id, $currentTheme) ?? SetNumberFormat($final) }}</span>
                    </li>
                </ul>
