                    ...
                    <div class="mini-cart-item">
                        ...
                        <div class="mini-cart-details">
                            ...
                            <div class="pvarprice d-flex align-items-center justify-content-between">
                                <div class="price">
                                    <ins>{{ currency_format_with_sym(($item->final_price ?? 0) , $store->id, $currentTheme) ?? SetNumberFormat($item->final_price) }} </ins>
                                </div>
                                <a class="remove_item_from_cart" title="Remove item" data-id="{{ $item->cart_id }}">
        ...
        <div class="mini-cart-footer">
            ...
            <div class="mini-cart-footer-total-row d-flex align-items-center justify-content-between">
                ...
                <div class="mini-total-price subtotal">
                    {{ currency_format_with_sym(($response->data->final_price ?? 0) , $store->id, $currentTheme) ?? SetNumberFormat($response->data->final_price ?? ($response->data->sub_total ?? 0)) }}
                </div>
            </div>
            @if($plan->shipping_method == 'on')
                <div class="u-save d-flex justify-end">
                    {{ __('Shipping') }} : <span class="CURRENCY"></span> <span class="shipping_final_price"> - {{ currency_format_with_sym((0 ?? 0) , $store->id, $currentTheme) ?? SetNumberFormat(0) }} </span>
                </div>
            @endif
            ...
            <div class="u-save d-flex justify-end" >
                {{ __('Total') }} : <span @if ($plan->shipping_method == 'on') class="CURRENCY" @else class="" @endif></span> <span class="final_amount_currency shipping_total_price" final_total="{{ $response->data->final_price  }}">{{ currency_format_with_sym(($response->data->final_price ?? 0) , $store->id, $currentTheme) ?? SetNumberFormat($response->data->final_price) }}</span>
            </div>
